...
Global ancestry analysis was performed by merging the Maghrib cohort with 1000 genome reference panels using shared SNPs. Shared genomic regions were first identified, and both datasets were filtered to retain matching variants. The filtered Maghrib and reference panel datasets were then merged using PLINK. Standard quality control was applied to remove samples and variants with high missingness, low-frequency variants, and variants deviating from Hardy–Weinberg equilibrium. The merged dataset was then LD-pruned prior to relatedness estimation. Pairwise kinship estimates were generated using KING and used for downstream principal component analysis accounting for relatedness.
...
Principal component analysis (PCA) was performed using the GENESIS R package to infer population structure while accounting for sample relatedness. The LD-pruned PLINK dataset was first converted to GDS format using SNPRelate. KING-robust kinship estimates were calculated to quantify ancestry divergence, while pairwise IBD segment estimates generated by KING were used to construct the kinship matrix. PCA was then performed using the PCAiR algorithm with a kinship threshold corresponding to third-degree relatives (2<sup>−9/2</sup>), enabling ancestry inference while minimizing the effects of cryptic relatedness. Principal components were merged with sample metadata and visualized using ggplot2, with individuals colored by population and shaped according to lifestyle. The proportion of variance explained by each principal component was calculated and displayed on the corresponding PCA axes.
